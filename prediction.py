...
data = pd.read_csv(f"WorldsSwiss - Blad1 (1).csv")
...
for i in range(length):
    ...
    # skip if draw
    # to implement draw take mean of one win and one loss
    else:
        continue
    
    # if home win gain 95% of skill level if away win gain 105% of skill level
    # take score into consideration: use sigmoid
    p_t = Normal_Distribution([teams[team1][0] - teams[team2][0]], beta + teams[team1][1] - teams[team2][1])
    t_sample = p_t.generate_samples(1000)
    y_sample = np.sign(t_sample)
    if np.mean(y_sample) > -0:
        y_predict = 1
    elif np.mean(y_sample) < -0:
        y_predict = -1
    ...
    if y_predict == y:
        ...
        print(f"Team1: {team1}, Team2: {team2}, Outcome: {y}, Prediction: {y_predict}, Certainty: {abs(np.mean(y_sample))*100}%")
...
print(f"Prediction rate = {100*correct/total}")
max_length = max(len(team) for team in teams)

for team, values in sorted(teams.items(), key=lambda item: item[1][0], reverse=True):
...
sorted_teams= sorted(teams.items(), key=lambda x:x[1][0], reverse=False)
fig, axs = plt.subplots(1, 5, figsize=(12, 6))  # 1 row, 3 columns
#print("The 5 teams with the lowest skill level:")
for i in range(5):
    #print(f"{sorted_teams[i][0]}:   \t mean = {sorted_teams[i][1][0]}, var = {sorted_teams[i][1][1]}")
    samples = np.random.normal(sorted_teams[i][1][0], sorted_teams[i][1][1], 1000)
    axs[i].hist(samples, bins=50, label="s1", alpha=0.5, density=True, color=colors[i])
...
with open("post_swiss_teams.pkl", "wb") as f:
    pickle.dump(teams, f)